...
//STACK - putem reprezenta o stiva prin LSI, LDI sau vector
...
//QUEUE - putem reprezenta o coada prin LSI, LDI sau vector
...
Masina getMasinaByID(Queue* queue, int id) {
	...
	while (queue->start != NULL) {
		...
		if (masina.id == id) {
			rezultat = masina;
			rezultat.model = malloc(strlen(masina.model) + 1);
			strcpy(rezultat.model, masina.model);
			rezultat.numeSofer = malloc(strlen(masina.numeSofer) + 1);
